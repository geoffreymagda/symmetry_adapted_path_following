...
class Path_Follower(object):
    ...
    def run(self,nls,x0,nsteps = 60, step_size=6e-3, initial_direction = 1, do_crit_pt = True, path_id = 0,name="no description",criteria = 1e-12,tan_reduc=0,reduced_step_size=3*1e-3):
        tstart = time.time()
        ...
        while (step < nsteps and len(self.crit_pts)<3):
            ...
            if (step == 0) and (initial_direction is not None)and (initial_direction is not 1)and (initial_direction is not -1):
                ...
                _,s,vh = np.linalg.svd(nls.get_J()[:-1,:])
            ...
            if last_tan is None:
                ...
                elif  initial_direction==1 and tan[-1] < 0:
                    tan = -1.0 * tan
                elif initial_direction==-1 and tan[-1] > 0 :
                    tan = -1.0 * tan
            ...
            if do_crit_pt:
                if state==State.PATHFOLLOW:
                    ...
                    if num_pos_eigs >= self.num_pos_eigs:
                        ...
                        maxstepsize = step_size
                    if maxstepsize - minstepsize < 1e-12:
                        J = nls.get_J()[:,:]
                        w,v = np.linalg.eig(J)
                        for i in range(w.shape[0]):
                            if np.abs(w[i])<1e-7:
