...
{
	public:
        compstripProblem(double l, double e,double a, double R, double nu, double mu0, double kappa,unsigned int refine, unsigned int n1, unsigned int n2);
		~compstripProblem();
...
compstripProblem::compstripProblem(double l_in, double e_in, double a_in, double R_in,double nu_in, double mu0_in, double kappa_in, unsigned int refine_in, unsigned int n1_in, unsigned int n2_in):
	...
	n2(n2_in)
{
	...
	Triangulation<2> triangulation_low_joint3;

	Triangulation<2> triangulation_high_right;
	Triangulation<2> triangulation_high_left;
	Triangulation<2> triangulation_high_high;
	Triangulation<2> triangulation_high_joint1;
	Triangulation<2> triangulation_high_joint2;
	Triangulation<2> triangulation_high_joint3;
	

	const double xb = -e/2;
	const double yb = 0.;
	const double xd = a*std::sqrt(3)*0.5 +0.25*e;
	const double yd = 0.5*l +a/2 -0.25*e*std::sqrt(3);

	const Point<2> Bl(xb,yb);
	const Point<2> Bh(xb,3*l-yb);
	...
	const std::array< Tensor<1,2>,2> & edgesBl {edgeBl1,edgeBl2};
	const std::array< Tensor<1,2>,2> & edgesBh {edgeBh1,edgeBh2};

	const std::vector< unsigned int > &  	subdivisions_two_n {2,n1};
	const std::vector< unsigned int > &  	subdivisions_n_two {n1,2};

	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_low_low,Bl,edgesBl,subdivisions_two_n, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_high_high,Bh,edgesBh,subdivisions_n_two, false);
	...
	edgeDlr1[0] = (0.5*l-a)*0.5*std::sqrt(3);	
	edgeDlr1[1] = (0.5*l-a)*0.5;
	edgeDlr2[0] = -0.5*e;
	edgeDlr2[1] = 0.5*std::sqrt(3.)*e;

	edgeDll1[0] = -edgeDlr1[0];
	edgeDll1[1] = edgeDlr1[1];	
	edgeDll2[0] = -edgeDlr2[0];
	edgeDll2[1] = edgeDlr2[1];

	edgeDhr1[0] = edgeDlr1[0];	
	edgeDhr1[1] = -edgeDlr1[1];
	edgeDhr2[0] = edgeDlr2[0];
	edgeDhr2[1] = -edgeDlr2[1];

	edgeDhl1[0] = -edgeDlr1[0];	
	edgeDhl1[1] = -edgeDlr1[1];
	edgeDhl2[0] = -edgeDlr2[0];
	edgeDhl2[1] = -edgeDlr2[1];
	...
	GridGenerator::merge_triangulations (triangulation_low_low,triangulation_low_right,triangulation);
	GridGenerator::merge_triangulations (triangulation_low_left,triangulation,triangulation); 
	GridGenerator::merge_triangulations (triangulation_high_high,triangulation,triangulation); 
	GridGenerator::merge_triangulations (triangulation_high_right,triangulation,triangulation); 
	GridGenerator::merge_triangulations (triangulation_high_left,triangulation,triangulation);
	...
	double xr=std::sqrt(3)*0.5*R;
	double yr=0.5*l-0.5*R;
	Point<2> Rl1(xr,yr);
	...
	//path low right boundary
	const double ux=0.5*e-xr;
	const double uy=0.5*l-a-yr;
	const double vx=-xr;
	const double vy=0.5*l-yr;
	const double alpha = -2*std::acos( (ux*vx+uy*vy) / (std::sqrt(ux*ux+uy*uy)*std::sqrt(vx*vx+vy*vy))	); 
	printf("alpha %f",alpha);
	for (unsigned int i =0; i< 2*n2+1; i++ ) {
		vertices[i]=(Rot(i*alpha/(2*n2),Pl1,Rl1));
		printf("vertice %i x %f y %f",i,vertices[i][0],vertices[i][1]);			
	}
	for (unsigned int i =0; i<n2+1; i++ ) {		
		vertices[2*n2+1+i]=( Point<2> (0,yp+(a*i/n2)) );	
		printf("vertice %i x %f y %f",2*n2+1+i,vertices[2*n2+1+i][0],vertices[2*n2+1+i][1]);		
	}

	for (unsigned int i =1; i< n2+1; i++ ) {		
		vertices[3*n2+1+i]=( Point<2> (0.5*a*std::sqrt(3)*i/n2,0.5*l+0.5*a*i/n2) );
	...
	Tensor<1,2> shift_low_to_origin;
	shift_low_to_origin[0]=0;
	shift_low_to_origin[1]=-0.5*l;
	Tensor<1,2> shift_origin_to_low;
	shift_origin_to_low[0]=0;
	shift_origin_to_low[1]=0.5*l;
	Tensor<1,2> shift_origin_to_high;
	shift_origin_to_high[0]=0;
	shift_origin_to_high[1]=2.5*l;

	triangulation_low_joint2.copy_triangulation(triangulation_low_joint1);
	GridTools::shift(shift_low_to_origin,triangulation_low_joint2);
	GridTools::rotate(2*PI/3,triangulation_low_joint2);
	triangulation_low_joint3.copy_triangulation(triangulation_low_joint2);
	GridTools::rotate(2*PI/3,triangulation_low_joint3);

	triangulation_high_joint1.copy_triangulation(triangulation_low_joint2);
	GridTools::rotate(PI/3,triangulation_high_joint1);
	triangulation_high_joint2.copy_triangulation(triangulation_high_joint1);
	triangulation_high_joint3.copy_triangulation(triangulation_high_joint1);

	GridTools::rotate(2*PI/3,triangulation_high_joint2);
	GridTools::rotate(4*PI/3,triangulation_high_joint3);

	GridTools::shift(shift_origin_to_low,triangulation_low_joint2);
	GridTools::shift(shift_origin_to_low,triangulation_low_joint3);

	GridTools::shift(shift_origin_to_high,triangulation_high_joint1);
	GridTools::shift(shift_origin_to_high,triangulation_high_joint2);
	GridTools::shift(shift_origin_to_high,triangulation_high_joint3);




	GridGenerator::merge_triangulations (triangulation_low_joint1,triangulation,triangulation);
	GridGenerator::merge_triangulations (triangulation_low_joint2,triangulation,triangulation);
	GridGenerator::merge_triangulations (triangulation_low_joint3,triangulation,triangulation);
	GridGenerator::merge_triangulations (triangulation_high_joint1,triangulation,triangulation);
	GridGenerator::merge_triangulations (triangulation_high_joint2,triangulation,triangulation);
	GridGenerator::merge_triangulations (triangulation_high_joint3,triangulation,triangulation);
...
PYBIND11_MODULE(compstrip, m) {
	py::class_<compstrip::compstripProblem>(m, "compstripProblem",py::buffer_protocol())
	 .def(py::init<double, double, double, double, double, double, double, unsigned int, unsigned int, unsigned int>
	  (),py::arg("l_in"), py::arg("e_in"),py::arg("a_in"), py::arg("R_in"), py::arg("nu_in"), py::arg("mu0_in"),py::arg("kappa_in"),py::arg("refine_in"), py::arg("n1_in"), py::arg("n2_in"))
