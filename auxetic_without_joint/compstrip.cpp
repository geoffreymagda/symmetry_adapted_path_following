...
{
	public:
        compstripProblem(double l, double e,double a, double Lenght, double nu, double mu0, double kappa, unsigned int n, unsigned int refine);
		~compstripProblem();
...
compstripProblem::compstripProblem(double l_in, double e_in,double a_in, double Lenght_in, double nu_in, double mu0_in, double kappa_in, unsigned int n_in, unsigned int refine):
	...
	n(n_in)
{
	...
	Triangulation<2> triangulation_high_high_rignt;
	Triangulation<2> triangulation_high_rignt;

	Triangulation<2> triangulation_high_high_left;
	Triangulation<2> triangulation_high_left;

	Triangulation<2> merge;

	e=std::sqrt( (Lenght*Lenght*0.25+a*a) / ( n*n*(0.25+a*a/(Lenght*Lenght)) ) );
	...
	double alpha = (0.5*l-delta)/(0.5*l-a*e/(Lenght)-delta);
	double alphabis = (0.5*l-l+delta)/(0.5*l+a*e/Lenght-l+delta);
	double beta = delta*(1-alpha);
	double betabis = (l-delta)*(1-alphabis);
	
	//building the points A and N
	printf("%f",delta);
	const double xa = 0.;
	const double ya = -delta;
	const double xn = 0;
	const double yn = delta;
	const Point<2> Al(xa,ya);
	const Point<2> Ah(xa,l-ya);
	const Point<2> Nl(xn,yn);
	const Point<2> Nh(xn,l-yn);
	...
	edgeAlr1[0] = Lenght/2.0;	
	edgeAlr1[1] = a;	
	edgeAlr2[0] = 0.;	
	edgeAlr2[1] = 2*delta;
	edgeNlr1[0] = e/2.;	
	edgeNlr1[1] = a*e/Lenght;	
	edgeNlr2[0] = 0;
	edgeNlr2[1] = l*0.5-a*e/Lenght-delta;

	edgeAll1[0] = -edgeAlr1[0];	
	edgeAll1[1] = edgeAlr1[1];	
	edgeAll2[0] = -edgeAlr2[0];	
	edgeAll2[1] = edgeAlr2[1];
	edgeNll1[0] = -edgeNlr1[0];	
	edgeNll1[1] = edgeNlr1[1];	
	edgeNll2[0] = -edgeNlr2[0];
	edgeNll2[1] = edgeNlr2[1];

	edgeAhl1[0] = -edgeAlr1[0];	
	edgeAhl1[1] = -edgeAlr1[1];	
	edgeAhl2[0] = -edgeAlr2[0];	
	edgeAhl2[1] = -edgeAlr2[1];
	edgeNhl1[0] = -edgeNlr1[0];	
	edgeNhl1[1] = -edgeNlr1[1];	
	edgeNhl2[0] = -edgeNlr2[0];
	edgeNhl2[1] = -edgeNlr2[1];

	edgeAhr1[0] = edgeAlr1[0];	
	edgeAhr1[1] = -edgeAlr1[1];	
	edgeAhr2[0] = edgeAlr2[0];	
	edgeAhr2[1] = -edgeAlr2[1];
	edgeNhr1[0] = edgeNlr1[0];	
	edgeNhr1[1] = -edgeNlr1[1];	
	edgeNhr2[0] = edgeNlr2[0];
	edgeNhr2[1] = -edgeNlr2[1];
	...
	const std::array< Tensor<1,2>,2> & edgesAhr {edgeAhr2,edgeAhr1};
	const std::array< Tensor<1,2>,2> & edgesNhr {edgeNhr2,edgeNhr1};

	const std::array< Tensor<1,2>,2> & edgesAhl {edgeAhl1,edgeAhl2};
	const std::array< Tensor<1,2>,2> & edgesNhl {edgeNhl1,edgeNhl2};
	...
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_low_low_rignt,Al,edgesAlr,subdivisions_n_two, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_low_rignt,Nl,edgesNlr,subdivisions_one_n, false);

	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_low_low_left,Al,edgesAll,subdivisions_two_n, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_low_left,Nl,edgesNll,subdivisions_n_one, false);

	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_high_high_rignt,Ah,edgesAhr,subdivisions_two_n, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_high_rignt,Nh,edgesNhr,subdivisions_n_one, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_high_high_left,Ah,edgesAhl,subdivisions_n_two, false);
	GridGenerator::subdivided_parallelepiped<2,2> (triangulation_high_left,Nh,edgesNhl,subdivisions_one_n, false);

	//GridTools::transform(const Transformation &  	transformation,triangulation_low_rignt) 	

  	for (const auto &cell : triangulation_low_rignt.active_cell_iterators())
    		{
          		Point<2> &v = cell->vertex(2);
          		if (v(0)==0)
				{
				printf("i %i v(0) %f v(1) %f alpha*v(1)+beta %f",4,v(0),v(1),alpha*v(1)+beta);
            			v(1) = alpha*v(1)+beta;
				}
    		}

  	for (const auto &cell : triangulation_low_left.active_cell_iterators())
    		{
          		Point<2> &v = cell->vertex(1);
          		if (v(0)==0)
            			v(1) = alpha*v(1)+beta;
    		}


  	for (const auto &cell : triangulation_high_rignt.active_cell_iterators())
    		{
          		Point<2> &v = cell->vertex(1);
          		if (v(0)==0)
            			v(1) = alphabis*v(1)+betabis;
    		}
  	for (const auto &cell : triangulation_high_left.active_cell_iterators())
    		{
          		Point<2> &v = cell->vertex(2);
          		if (v(0)==0)
            			v(1) = alphabis*v(1)+betabis;
	...
	GridGenerator::merge_triangulations (triangulation_low_low_rignt,triangulation_low_rignt,merge);
	GridGenerator::merge_triangulations (triangulation_low_low_left,merge,merge);
	GridGenerator::merge_triangulations (triangulation_low_left,merge,merge);


	GridGenerator::merge_triangulations (triangulation_high_high_rignt,merge,merge);
	GridGenerator::merge_triangulations (triangulation_high_rignt,merge,merge);
	GridGenerator::merge_triangulations (triangulation_high_high_left,merge,merge);
	GridGenerator::merge_triangulations (triangulation_high_left,merge,triangulation);
	...
	constraints.clear();
	{
		...
		for (; cell!=endc; ++cell)
		{
			...
			for(unsigned int i=0; i< 4 ;++i)
			{
				...
				if(std::abs(-1*xa + edgeAhl1[0]+edgeAhl2[0]-coords(0))<1e-4 && std::abs(l-ya + edgeAhl1[1]+edgeAhl2[1]-coords(1))<1e-4)
				{
				...
				if(std::abs(-1*xa + edgeAhl1[0]+0.5*edgeAhl2[0]-coords(0))<1e-4 && std::abs(l-ya + edgeAhl1[1]+0.5*edgeAhl2[1]-coords(1))<1e-4)
				{
				...
				if(std::abs(-1*xa + edgeAhl1[0]-coords(0))<1e-4 && std::abs(l-ya + edgeAhl1[1]-coords(1))<1e-4)
				{
				...
				if(std::abs(xa +edgeAhr1[0]-coords(0))<1e-4 && std::abs(l-ya +edgeAhr1[1]-coords(1))<1e-4)
				{
				...
				if(std::abs(xa + edgeAhr1[0]+0.5*edgeAhr2[0]-coords(0))<1e-4 && std::abs(l-ya + edgeAhr1[1]+0.5*edgeAhr2[1]-coords(1))<1e-4)
				{
				...
				if(std::abs(xa +edgeAhr2[0] + edgeAhr1[0]-coords(0))<1e-4 && std::abs(l-ya +edgeAhr2[1] + edgeAhr1[1]-coords(1))<1e-4)
				{
...
PYBIND11_MODULE(compstrip, m) {
	py::class_<compstrip::compstripProblem>(m, "compstripProblem",py::buffer_protocol())
	 .def(py::init<double, double,double, double, double, double, double, unsigned int, unsigned int>
	  (),py::arg("l_in"), py::arg("e_in"),py::arg("a_in"), py::arg("Lenght_in"), py::arg("nu_in"), py::arg("mu0_in"),py::arg("kappa_in"), py::arg("n_in"), py::arg("refine"))
